...
fn prepare_cflags(cflags: &str, kernel_dir: &str) -> Vec<String> {
    ...
    while let Some(arg) = cflag_iter.next() {
        if arg.starts_with("-I") && !arg.starts_with("-I/") {
            ...
            // Ejemplos: -I./arch/x86/include to -I/lib/modules/5.8.0-43-generic/build/./arch/x86/include
            kernel_args.push(format!("-I{}/{}", kernel_dir, &arg[2..]));
        } else if arg == "-include" {
            // Convert the -includes commands
            // Ejemplos: -include ./include/linux/kconfig.h 
            // to -include /lib/modules/5.8.0-43-generic/build/./include/linux/kconfig.h
...
fn main() {
    ...
    builder.compiler("clang-11".to_string());
